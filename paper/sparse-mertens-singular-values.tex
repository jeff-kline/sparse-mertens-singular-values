...
\[
  ...
  |M(n)|\cdot n^{-4/3+o(1)},
\]
and, established separately,
$\mathrm{RH}\iff\sigma_{\min}(\Rc_n)\ll_\varepsilon n^{-1+\varepsilon}$,
two-sided and unconditional. The forward implication does not use the displayed
upper bound, which under RH would give only $n^{-5/6+\varepsilon}$; it uses the
collapsed equality $\sigma_{\min}(\Rc_n)=|M(n)|n^{-3/2+o(1)}$, valid as soon as
