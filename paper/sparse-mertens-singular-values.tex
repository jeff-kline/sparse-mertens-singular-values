...
two-sided and unconditional. The bracket collapses to
$\sigma_{\min}(\Rc_n)=|M(n)|n^{-3/2+o(1)}$ once $|M(n)|/\|w\|\to0$ --- which RH
supplies, and which is not known unconditionally. The mechanism is that the entries of the vector
...
and $\sigma_{\min}\ll n^{-1+2\varepsilon}$. It is worth noting that the bracket
