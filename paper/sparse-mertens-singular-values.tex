...
\date{Release candidate 0.1.0, August 2026}

\begin{document}
\maketitle

\begin{abstract}
Redheffer's $(0,1)$ matrix has determinant $M(n)=\sum_{k\le n}\mu(k)$. We study a
much sparser $(0,1)$ matrix $\Rc_n$ with the same determinant: it has about
$2.61n$ nonzero entries rather than order $n\log n$. We prove that its smallest
singular value satisfies, unconditionally,
\[
  |M(n)|\cdot n^{-3/2+o(1)}\ \le\ \sigma_{\min}(\Rc_n)\ \le\
  |M(n)|\cdot n^{-4/3+o(1)},
\]
and that
$\mathrm{RH}\iff\sigma_{\min}(\Rc_n)\ll_\varepsilon n^{-1+\varepsilon}$.
The equivalence is two-sided and unconditional. The key identity expresses the
coordinates of $w=\Ac^{-\mathsf T}(\mathbf 1-e_1)$, for the unit
lower-triangular factor $\Ac$, as rough-M\"obius sums
$M(x,y)=\sum_{d\le x,\,\Pm(d)>y}\mu(d)$. Alladi's 1982 asymptotic for these sums
\cite{alladiJNT} then gives $\|w\|=n^{1+o(1)}$. Under the precise rank-one
dominance condition
\[
 \frac{|M(n)|\,\|\Ac_n^{-1}\|}{\|\mu_n\|\,\|w\|}\longrightarrow0,
\]
which follows from RH, the bracket sharpens to
$\sigma_{\min}(\Rc_n)=|M(n)|n^{-3/2+o(1)}$. At the other end we prove
$\sigma_{\max}(\Rc_n)=\sqrt n\,(1+O(1/n))$ and
$\sigma_{\max}/|\lambda_{\max}|\asymp\sqrt{\log n}$. We also prove
$\|\Ac_n^{-1}\|=n^{1/2+o(1)}$, hence
$\sigma_{\min}(\Ac_n)=n^{-1/2+o(1)}$, which answers a question of Cheon and Kim
for this family. The new contribution is the linear-algebraic connection and
its consequences; the sparse matrix, determinant identity, and analytic
rough-M\"obius estimate are prior work.
\end{abstract}

\section{Introduction}\label{sec:intro}

This paper connects the smallest singular value of a sparse $(0,1)$ matrix to
the size of the Mertens function. Write $\mu$ for the M\"obius function and
\[
  M(n) := \sum_{k\le n}\mu(k)
\]
for the Mertens function. The main result is the unconditional bracket
\[
 |M(n)|n^{-3/2+o(1)}\le \sigma_{\min}(\Rc_n)
 \le |M(n)|n^{-4/3+o(1)}
\]
for the sparse matrix $\Rc_n$ defined below, together with the equivalence
\[
 \mathrm{RH}\quad\Longleftrightarrow\quad
 \sigma_{\min}(\Rc_n)\ll_\varepsilon n^{-1+\varepsilon}.
\]
The proof identifies a vector in $\Rc_n^{-1}$ with M\"obius sums over integers
free of small prime factors and then applies a theorem of Alladi
\cite{alladiJNT}. The matrix identity is proved here; the analytic estimate is
prior work.

It is classical that the prime number theorem is equivalent to $M(n)=o(n)$,
and that the Riemann hypothesis is equivalent to
...
\cite{clementsteinerberger} treat the largest singular value of Redheffer's
matrix. Hilberdink \cite[\S4(b)]{hilberdink} proves an
$n^{-1/2}$ smallest-singular-value asymptotic for the dense divisibility
triangular factor underlying Redheffer's matrix. That result is close in
exponent but does not treat the sparse forest factor $\Ac_n$ or the modified
matrix $\Rc_n$. The idea of routing the Riemann hypothesis through a smallest singular
value appears to originate with Bordell\`es and Cloitre
\cite{bordellescloitre}, who study an
...
and in neither is the relevant singular value determined. Here the modified
matrix yields a two-sided equivalence, while the unmodified triangular factor
fails the Cheon--Kim condition. These are statements about this particular
sparse family, not a general singular-value criterion for Mertens matrices.

\subsection{The matrix}

The following much sparser $(0,1)$ matrix with the same determinant was
introduced in \cite{kline581} and restated in \cite[\S1]{kline584}. Let
$\Pp(i)$ denote the largest prime factor
of $i>1$, and set
...
\[
  ...
  |M(n)|\cdot n^{-4/3+o(1)},
\]
the upper bound using the classical quantitative prime number theorem (Remark
\ref{rem:hierarchy}). If moreover
\[
 \frac{|M(n)|\,\|\Ac_n^{-1}\|}{\|\mu_n\|\,\|w\|}\longrightarrow0,
\]
the two collapse to
...
The equivalence is two-sided and unconditional. The implication
``$\Longleftarrow$'' is the unconditional lower bound rearranged. For
``$\Longrightarrow$'', RH makes the displayed rank-one dominance condition hold
and hence supplies the exact asymptotic. The simpler condition
$|M(n)|/\|w\|\to0$ is not used as a substitute: the proof also has to control
$\|\Ac_n^{-1}\|/\|\mu_n\|=n^{o(1)}$.
...
forward implication, and hence the full equivalence, is unavailable at the
stated exponent.
...
$\|\Ac_n^{-1}\|/\sqrt n$ is nearly constant over the dense-matrix range reported
in \S\ref{sec:numerics}, suggesting $\|\Ac_n^{-1}\|\asymp\sqrt n$; we do not
prove that sharper estimate.
...
|M(n)|n^{-1/2}$. Under the rank-one dominance condition of Theorem
\ref{thm:main}, the comparison becomes
...
is the minimum over all unit directions. In the dominance regime they differ by
$\|w\|=n^{1+o(1)}$. The unconditional bracket retains the same lower exponent
but does not assert this exact comparison at every $n$.
...
numerical claims are reproduced by the scripts in \texttt{code/}. The bounded
literature record and the scope of earlier AI-assisted checks are recorded in
\texttt{audit/reports/}; those checks are not peer review.
...
$\|w\|^2\le\sum_{j\le n}(n/j)^2\le\zeta(2)n^2$. Therefore
$\|w\|\le (\pi/\sqrt6)\,n$.
...
dominates the error $O(x\alpha^2/\log^2 y)$ by a factor $\log y$; the other
main term $y/\log y$ is lower order because
$y/x=n^{-(\alpha-1)/(\alpha+1)+o(1)}$. Since
...
\[
 \|w\| = n\exp\bigl(-(1+o(1))\sqrt{\log n\log\log n}\,\bigr),
\]
with the maximising subfamily at
$\alpha\asymp\sqrt{\log n/\log\log n}$. This is \emph{not}
...
\end{equation}
Here the limits are understood along indices with $M(n)\ne0$. At an index with
$M(n)=0$, the matrix $\Rc_n$ is singular and the corresponding zero-valued
...
Theorem \ref{thm:max}. Condition \eqref{eq:dominance} says exactly that the norm
of $\Ac_n^{-1}$ is negligible relative to the rank-one term. The reverse
triangle inequality therefore gives
$\|\Rc_n^{-1}\|=\|\mu_n\|\|w\|/|M(n)|\,(1+o(1))$, and
$\sigma_{\min}(\Rc_n)=1/\|\Rc_n^{-1}\|$ gives
\eqref{eq:factorisation}.
\end{proof}

RH supplies \eqref{eq:dominance}: for any fixed $0<\varepsilon<1/2$ its left-hand
side is $n^{-1/2+\varepsilon+o(1)}$. The condition is not known unconditionally.
This is why Theorem \ref{thm:main} gives an unconditional bracket rather than an
unconditional asymptotic.

\begin{proof}[Proof of Theorem \ref{thm:main}]
If $M(n)=0$ then $\Rc_n$ is singular and the two bounds are immediate, so assume
$M(n)\ne0$.
By Theorem \ref{thm:588}, $\|\mu_n\| = \sqrt{6n/\pi^2}\,(1+o(1)) = n^{1/2+o(1)}$,
and by Theorem \ref{thm:norm}, $\|w\|=n^{1+o(1)}$.

\emph{Lower bound.} From the inverse formula and the triangle inequality,
\[
 \sigma_{\min}(\Rc_n)
 \ge \frac{|M(n)|}{|M(n)|\,\|\Ac_n^{-1}\|+\|\mu_n\|\,\|w\|}.
\]
The trivial bound $|M(n)|\le n$, Theorem \ref{thm:max}, and the elementary bounds
$\|w\|\le(\pi/\sqrt6)n$ and $\|\mu_n\|\le\sqrt n$ make the denominator
$n^{3/2+o(1)}$. Thus
$\sigma_{\min}(\Rc_n)\ge|M(n)|n^{-3/2+o(1)}$ unconditionally.
\emph{Upper bound.} Remark \ref{rem:hierarchy} gives
$\sigma_{\min}(\Rc_n)\ll|M(n)|n^{-4/3+o(1)}$ from the classical
zero-free-region bound for the Mertens function.
If \eqref{eq:dominance} holds, Proposition \ref{prop:sm} applies and substituting
the two norms into \eqref{eq:factorisation} collapses the bracket to
$\sigma_{\min}(\Rc_n)=|M(n)|n^{-3/2+o(1)}$.

For the equivalence: if RH holds then $M(n)\ll n^{1/2+\varepsilon}$; together
with $\|\Ac_n^{-1}\|=n^{1/2+o(1)}$, $\|\mu_n\|=n^{1/2+o(1)}$, and
$\|w\|=n^{1+o(1)}$, this gives \eqref{eq:dominance}. The exact asymptotic is
therefore available and $\sigma_{\min}\ll n^{-1+2\varepsilon}$. It is worth noting that the bracket
alone does not give this: its upper bound combined with RH yields only
$\sigma_{\min}\ll n^{1/2+\varepsilon}n^{-4/3+o(1)}=n^{-5/6+\varepsilon+o(1)}$,
which is weaker than $n^{-1+\varepsilon}$. The forward implication genuinely
needs the collapse, and hence Theorem \ref{thm:norm}. Conversely,
...
\begin{remark}
$\Rc_n$ is singular exactly when $M(n)=0$. Thus no positive lower bound can hold
at those indices; the lower side of Theorem \ref{thm:main} is scaled by
$|M(n)|$. Test vectors and projections provide the complementary upper bounds;
see Remark \ref{rem:hierarchy}.
\end{remark}

\begin{remark}[a hierarchy of cheaper bounds]\label{rem:hierarchy}
Assume $M(n)\ne0$; when $M(n)=0$ the asserted upper bounds are immediate.
...
$\sigma_{\min}\le|M(n)|/\|\mu_n\|$. Under \eqref{eq:dominance}, comparison with
\eqref{eq:factorisation} shows that this is weaker by a factor
$\|w\|(1+o(1))$.

For an unconditional sharper bound, let $Q$ project onto prime coordinates
$p\in[n^{1/3},2n^{1/3}]$. Corollary \ref{cor:trichotomy} and the prime number
theorem give
\[
 |w_p|=\pi(n/p)-\pi(p)-1=n^{2/3+o(1)},\qquad
 \|Qw\|=n^{5/6+o(1)}.
\]
The closed form for $\Ac_n^{-1}$ shows that its column indexed by $p$ is
supported on squarefree $k$ whose least prime factor is $p$. These supports are
disjoint for distinct primes, and therefore
\[
 \|\Ac_n^{-1}Q\|
 =\max_p\sqrt{1+\pi(n/p)-\pi(p)}=n^{1/3+o(1)}.
\]
Using the inverse formula, the reverse triangle inequality, and the classical
zero-free-region bound \cite{leeleong}
$|M(n)|\ll n\exp(-c\sqrt{\log n})$, we obtain
the explicit domination ratio
\[
 \frac{|M(n)|\,\|\Ac_n^{-1}Q\|}
      {\|\mu_n\|\,\|Qw\|}
 \ll \frac{|M(n)|\log n}{n}=o(1).
\]
Consequently
\[
 \|\Rc_n^{-1}Q\|
 \ge \frac{\|\mu_n\|\,\|Qw\|}{|M(n)|}-\|\Ac_n^{-1}Q\|
 =\frac{n^{4/3+o(1)}}{|M(n)|}.
\]
Thus $\sigma_{\min}(\Rc_n)\ll|M(n)|n^{-4/3+o(1)}$. The final step from
$n^{-4/3}$ to the conditional $n^{-3/2}$ asymptotic requires the full estimate
for $\|w\|$ from Theorem \ref{thm:alladi}. Combining the unconditional upper
bound with the same zero-free-region estimate gives
$\sigma_{\min}(\Rc_n)\ll n^{-1/3+o(1)}$.
\end{remark}
...
$\|\Ac_n^{-1}\|\le\sum_r\sqrt{\pi_r'(n)}$. There are at most
$1+\lfloor\log_2 n\rfloor$ nonzero summands and
$\sum_r\pi_r'(n)\le n$, so Cauchy--Schwarz gives
$\sum_r\sqrt{\pi_r'(n)}\le\sqrt{(1+\lfloor\log_2 n\rfloor)n}
=n^{1/2+o(1)}$. Conversely
...
Theorem \ref{thm:max} is a case where the arithmetic structure determines the
exponent up to a subpolynomial factor: for the triangular factor $\Ac_n$
considered here, $\sigma_{\min}(\Ac_n)=n^{-1/2+o(1)}$, obtained not from a general matrix
...
For Theorem \ref{thm:max}, at $n=400,800,1600,3200,6400$ the values of
$\sigma_{\max}(\Rc_n)/\sqrt n$ are
\[
 1.00387,\quad 1.00191,\quad 1.00094,\quad 1.00046,\quad 1.00023,
\]
so the excess roughly halves at each doubling. At the same sizes,
$\sigma_{\max}/|\lambda_{\max}|$ equals
\[
 1.906,\quad 2.087,\quad 2.262,\quad 2.439,\quad 2.592,
\]
against $\sqrt{\log n}=2.448,\dots,2.960$. The nilpotency index of $S$ is
$5,5,5,6$ at $n=500,\dots,3000$; $\|\Ac_n^{-1}\|/\sqrt n$ equals
$0.853,0.851,0.849,0.850$; and $\sigma_{\min}(\Ac_n)\sqrt n$ equals
$1.1717,1.1753,1.1773,1.1760$. The quantity of Remark \ref{rem:cheonkim} equals
$427,851,1699,2552$; for comparison, $\sqrt n$ equals $22,32,45,55$ at those
sizes.
...
$n=10^5,\dots,10^7$: already drifting, and accelerating.
...
fails the Cheon--Kim criterion because its triangular factor satisfies
$\sigma_{\min}(\Ac_n)=n^{-1/2+o(1)}$. Is there a family in which
$\sigma_{\min}(L_n)\gg n^{-\varepsilon}$? Here the first column of
$\Ac_n^{-1}$ already supplies the lower bound $\|\Ac_n^{-1}\|\gg\sqrt n$; any
candidate family must avoid an analogous obstruction. The same question for
...
\bibitem{hilberdink}
T.~W.~Hilberdink, \emph{Singular values of multiplicative Toeplitz matrices},
...
\bibitem{leeleong}
E.~S.~Lee, N.~Leong, \emph{New explicit bounds for Mertens function and the
